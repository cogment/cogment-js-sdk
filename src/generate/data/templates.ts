...
${Object.keys(cogSettings?.import).forEach(key => {
  if (key === 'javascript' || key === 'python') {
    throw new Error("CogSettings.import.javascript and CogSettings.import.python are no longer supported in Cogment 2.0");
  }
})
  }
...
${(
    cogSettings.actor_classes as {
      ...
      config_type: string;
      action: { space: string };
      observation: { space: string; delta: string; deltaApplyFn: string };
    }[]
  )
    .map((actorClass) => {
      ...
      if (actorClass.observation.deltaApplyFn)
        throw new Error(
          `${actorClass.name} has an observation deltaApplyFn in the configuration file, this is no longer supported in Cogment 2.0, please remove this property`,
        );
