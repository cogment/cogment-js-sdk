...
export class Session<ActionT extends MessageBase, ObservationT extends MessageBase> {
    ...
    protected _start = (autoDoneSending: boolean) => {
        ...
        if (this._trial.ended)
            throw new Error("Cannot start [${this.name}] because the trial has ended.")
    ...
    private _newEvent = (event: RecvEvent<ActionT, ObservationT>) => {
        ...
        if (!event)
            console.warn(`Trial [${this._trial.id}] - Session for [${this.name}]: New event is undefined`)
    ...
    protected _postData = (data: Message | _Ending | _EndingAck) => {
        ...
        if (data instanceof _Ending)
            this._trial.ending = true
        else if (data instanceof _EndingAck)
            this._trial.endingAck = true
    ...
    public sendingDone = () => {
        if (this._autoAck)
            ...
            console.warn(`Trial [${this._trial.id}] - Session [${this.name}] end sending ignored because the trial has already ended.`)
        else if (this._trial.endingAck)
            console.warn(`Trial [${this._trial.id}] - Session [${this.name}] cannot end sending more than once`)
        else
