...
import {CogSettings} from 'cogment';
import * as data_pb2 from './data_pb.js';

const human_class = cog_settings.actor_classes.client;
...
async function start() {
  ...
  if (!trial) {
    try {
      ...
      trial = await conn.start_trial(cog_settings.actor_classes.client);
  ...
  console.log(
    `Client Trial now established, id: ${trial.id} + human id: ${human_id}`,
  );
